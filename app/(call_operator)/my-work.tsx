...
export default function MyWorkScreen() {
  ...
  const getWorkStats = () => {
    ...
    const todayLeads = myLeads.filter(lead => {
      ...
      return leadDate === today && lead.callOperatorId === user?.id;
    });
    ...
    return {
      ...
      totalLeads: myLeads.filter(l => l.callOperatorId === user?.id).length,
      totalTickets: myTickets.length,
      completedLeads: completedLeads.length,
      resolvedTickets: resolvedTickets.length,
      conversionRate: myLeads.length > 0 ? ((completedLeads.length / myLeads.filter(l => l.callOperatorId === user?.id).length) * 100).toFixed(1) : '0'
    };
  };
  ...
  const recentActivity = [
    ...myLeads
      .filter(l => l.callOperatorId === user?.id)
      .slice(0, 3)
      .map(lead => ({
        id: lead.id,
        type: 'lead',
        title: `Called ${lead.customerName}`,
        subtitle: `Status: ${lead.status}`,
        time: new Date(lead.updatedAt).toLocaleTimeString(),
        icon: Phone,
        color: '#1E40AF'
      })),
    ...myTickets
      ...
      .map(ticket => ({
        ...
        type: 'ticket',
        title: ticket.title,
        subtitle: `Customer: ${ticket.customerName}`,
