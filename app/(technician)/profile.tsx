...
import { User, Phone, Mail, LogOut, Wrench, TrendingUp, Calendar, CheckCircle, Clock } from 'lucide-react-native';
...
export default function TechnicianProfileScreen() {
  ...
  const todayVisits = myLeads.filter(lead => {
    const today = new Date().toDateString();
    const leadDate = new Date(lead.updatedAt).toDateString();
    return today === leadDate && lead.technicianId === user.id;
  });

  const completedInstallations = myLeads.filter(l => l.status === 'completed' && l.technicianId === user.id);
